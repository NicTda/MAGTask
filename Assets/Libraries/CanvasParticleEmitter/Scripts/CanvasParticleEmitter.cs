...
namespace UnityEngine.UI
{
	...
	public class CanvasParticleEmitter : MaskableGraphic, ISerializationCallbackReceiver {
        ...
        [SerializeField] private bool m_EmitOnce = false;
        public bool EmitOnce { get { return m_EmitOnce; } set { m_EmitOnce = value; } }
		...
		void UpdateEmitter(){
            ...
            if (m_EmitOnce && !m_Emitted)
            {
                AddParticle();
                m_Emitted = true;
            }
            else if(m_EmitOnce && m_Emitted)
            {
                m_isActive = false;
            }
            else
			...
			for (int i = 0; i < particlesToEmit; i++)
			{
				...
				case EmitterShapes.RectEmitter:
					...
					if (m_EmitterShapeRectTransform != null)
                        {
						tempRect = CanvasParticleHelper.GetRectCanvasSpace(m_EmitterShapeRectTransform);
					}
